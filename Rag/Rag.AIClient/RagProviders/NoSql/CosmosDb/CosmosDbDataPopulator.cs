...
namespace Rag.AIClient.RagProviders.NoSql.CosmosDb
{
	public class CosmosDbDataPopulator : IDataPopulator
	{
		public async Task LoadData()
		{
			...
			await this.CreateDocuments(@$"Data\{RagProviderFactory.GetCosmosDbConfig().JsonInitialDataFilename}", container);
		...
		{
			var containerName = RagProviderFactory.GetCosmosDbConfig().ContainerName;
		...
		private async Task CreateDocuments(string jsonFilename, Container container)
		{
			...
			{
				var task = container.CreateItemAsync(document, new PartitionKey(RagProviderFactory.GetCosmosDbConfig().PartitionKeyValue));
				tasks.Add(task
					.ContinueWith(t =>
					{
		...
		public async Task UpdateData()
		{
			...
			await this.CreateDocuments(@$"Data\{RagProviderFactory.GetCosmosDbConfig().JsonUpdateDataFilename}", container);      // Let the Azure Function wake to consume the change feed and vectorize the new documents
		}

...
{
	public class HikingBenefitsDemo : SemanticKernelDemoBase
	{
...
Remember, hiking can be tailored to your fitness level—start with shorter, easier trails and gradually progress. Lace up those hiking boots and embark on an adventure! 🌲🥾
			...
			var response = await kernel.InvokePromptAsync<string>(humanMessageText, new(new OpenAIPromptExecutionSettings() { MaxTokens = 400 }));

			base.WriteLine(response, ConsoleColor.Green);
		}
	}
}
